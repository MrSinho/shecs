...
static x* ezecsAdd ## x(void** scene, const uint32_t entity) { \
	ezecsCheckEntitiesSize(entity);\
	ezecsCheckComponentsSize(ezecs ## x ## ID);\
	x* component = (x*)calloc(1, sizeof(x));\
	scene[entity * EZ_ECS_MAX_ENTITIES + ezecs ## x ## ID] = (void*)component;\
	return component;\
} \
static const x* ezecsSet ## x(void** scene, const x* component, const uint32_t entity) {\
	ezecsCheckEntitiesSize(entity);\
	ezecsCheckComponentsSize(ezecs ## x ## ID);\
	scene[entity * EZ_ECS_MAX_ENTITIES + ezecs ## x ## ID] = (void*)component;\
	return component;\
}\
static int ezecsHas ## x(const void** scene, const uint32_t entity) { \
	return !(scene[entity * EZ_ECS_MAX_ENTITIES + ezecs ## x ## ID] == NULL); \
} \
static x* ezecsGet ## x(const void** scene, const uint32_t entity) { \
	return (x*)scene[entity * EZ_ECS_MAX_ENTITIES + ezecs ## x ## ID]; \
} \
static void ezecsRemove ## x(void** scene, const uint32_t entity) { \
	free(scene[entity * EZ_ECS_MAX_ENTITIES + ezecs ## x ## ID]);\
	scene[entity * EZ_ECS_MAX_ENTITIES + ezecs ## x ## ID] = NULL;\
} \


static void ezecsCreateScene(void** scene);

static const uint32_t ezecsCreateEntity();

static void ezecsDestroyEntity(void** scene, const uint32_t entity);
...
void ezecsCreateScene(void** scene) {
	for (uint32_t entity = 0; entity < EZ_ECS_MAX_ENTITIES; entity++) {
		for (uint32_t component = 0; component < EZ_ECS_MAX_COMPONENTS; component++) {
			scene[entity * EZ_ECS_MAX_ENTITIES + component] = NULL;
		}
	}
}
...
void ezecsDestroyEntity(void** scene, const uint32_t entity) {
	for (uint32_t component = 0; component < EZ_ECS_MAX_COMPONENTS; component++) {
